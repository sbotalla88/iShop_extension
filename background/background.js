...
class Background {
    ...
    onUpdatedTab = (tabId, changeInfo, tab) => {
        if (changeInfo.status == "complete") {
            ...
            ext.tabs.sendMessage(tab.id, { action: "add_save_btn" });
            // background.sendMessage(tab, "add_save_btn");
            // console.log("week: ", tab.url.indexOf("week"))
        }
    };
     ...
     * send message
     */
    sendMessage = (tab, msg, data = null) => {
        ext.tabs.sendMessage(tab.id, { action: msg, data });
    };
}
